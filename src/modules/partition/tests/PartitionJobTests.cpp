...
PartitionJobTests::testResizePartition()
{
    ...
    // Setup: create the test partition
    {
        ...
        QVERIFY( freePartition );
        Partition* partition = KPMHelpers::createNewPartition( freePartition->parent(), *m_device, PartitionRole( PartitionRole::Primary ), FileSystem::Ext4, testName, oldFirst, oldLast );
